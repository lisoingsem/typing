...
<template>
  <div ref="toolbar" class="relative shrink-0">
    <button
      ...
      :aria-label="`កំណត់លំហាត់៖ ${modeLabel} ${typing.parameter}`"
    ...
    <div v-if="open" class="absolute right-0 top-full z-30 mt-2 w-64 rounded-xl border border-faint bg-panel p-3 text-xs shadow-2xl" role="dialog" aria-label="កំណត់លំហាត់">
      <div class="mb-3 flex items-center justify-between">
        <p class="font-medium text-text">កំណត់លំហាត់</p>
        <span class="text-[9px] text-muted">{{ modeLabel }} · {{ typing.parameter }}</span>
      </div>

      <p class="mb-1.5 text-[9px] text-muted">របៀបហាត់</p>
      <div class="grid grid-cols-3 gap-1" role="group" aria-label="ជ្រើសរបៀបហាត់">
        <button
